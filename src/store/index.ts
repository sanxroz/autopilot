...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  checkGitHubCli: async () => {
    ...
    try {
      ...
      if (available) {
        try {
          cliAuthUser = await invoke<string>('check_gh_auth');
        } catch {}
      }
      ...
      set((state) => ({
        githubSettings: {
          ...
          ghAuthUser: cliAuthUser || oauthUser,
        },
      }));
    } catch (e) {
